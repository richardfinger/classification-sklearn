...
#  5. Display of classification report - comparing best found random forest to GaussianNB
...
def print_df_statistics(df):
  ...
  print("With percentages : ",[x/df.shape[0] for x in list(df.iloc[:,0].value_counts())])
...
def main():
  ...
  model  = RandomForestClassifier(class_weight='balanced',max_depth=20,max_features=0.8,min_samples_split=10,n_jobs=-1)
  pipeline_for_feature_selector = Pipeline([('scaler',scaler),('model',model)])
  ...
  feature_selector = SelectFromModel(pipeline_for_feature_selector.named_steps['model'],threshold=0.0012,prefit=True)
  ...
  pipeline = Pipeline([('scaler',scaler),('feature_selector',feature_selector),('model',model)])
  ...
  search = GridSearchCV(pipeline, cv=5, param_grid=param_grid, n_jobs=1, scoring=scoring_averaged,refit=True)
  ...
  model  = GaussianNB()
  pipeline_naive = Pipeline([('scaler',scaler),('feature_selector',feature_selector),('model',model)])
  ...
  for pipeline in pipelines:
    ...
    if 'feature_selector' in pipeline.named_steps:
      ...
      print("Number of features:",pipeline.named_steps['feature_selector'].transform(data).shape[1])
    ...
    y_train_pred_from_prob = threshold_proba(y_train_pred_prob,classes,threshold)
    ...
    print("F1 Score:", f1_score(y_train.values,y_train_pred_from_prob,average='macro'))
    ...
    y_test_pred_from_prob = threshold_proba(y_test_pred_prob,classes,threshold)
    ...
    print("F1 Score:", f1_score(y_test.values,y_test_pred_from_prob,average='macro'))
